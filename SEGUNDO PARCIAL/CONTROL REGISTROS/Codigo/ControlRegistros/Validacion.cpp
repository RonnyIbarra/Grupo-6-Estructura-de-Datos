...
std::string Validacion::validarDni() {
    ...
    if (digitoVerificadorCalculado == verificador) {
        return cedulaStr; // Retorna la cédula válida
    } else {
...
float Validacion::validarSueldo(){
	cout << "Ingrese el sueldo: " ;
	float sueldo = 0.0;
	sueldo = ingresarDatosfloat();
	while(sueldo < 450.0 || sueldo > 1000.0){
		cout << "\033[1;33mRango Permitido ($450-$1000)\033[0m" << endl;
		cout << "Ingrese el sueldo: " ;
		sueldo = ingresarDatosfloat();
	}
	return sueldo;
